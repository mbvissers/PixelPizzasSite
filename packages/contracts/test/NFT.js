...
describe("PixelPizzas contract", function () {
    ...
    it("Minting a single token should work if price is set to 0", async function () {
        ...
        expect(await fixture.pixelPizzas.balanceOf(fixture.alice.getAddress())).to.equal(1)
    });

    it("Minting multiple tokens should work if price is set to 0", async function () {
        const fixture = await loadFixture(deployTokenFixture);

        await fixture.pixelPizzas.connect(fixture.bob).safeMint(5, {value: ethers.utils.parseEther("10")})
        
        expect(await fixture.pixelPizzas.balanceOf(fixture.bob.getAddress())).to.equal(5)
    });
});